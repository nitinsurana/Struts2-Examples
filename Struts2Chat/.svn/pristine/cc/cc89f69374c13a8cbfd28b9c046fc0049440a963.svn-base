...
public class Chatterbot10 {
    ...
    private static String sContext = new String("");
    private static String sPrevContext = new String("");
    ...
    static String KnowledgeBase[][][]= {
        ...
        {{"SO THAT'S YOUR NAME"},
            {"YES, THAT'S MY NAME", "YES IT IS"},
            {"MY NAME IS CHATTERBOT", "YOU CAN CALL ME CHATTERBOX"}
        },
    ...
    public void loadDatabase(){
        try {
            BufferedReader br=new BufferedReader(new FileReader(new File("script.txt")));
            String s=null;
            List<String> keyList=new ArrayList<String>();
            CResponse respObj=new CResponse();
            List<CResponse> ListOfRespObj=new ArrayList<CResponse>();

            while((s=br.readLine())!=null){
                char cSymbol=s.charAt(0);
                s=s.substring(1);
                ...
                switch(cSymbol){
                    case 'K':
                        ...
                        break;
                    case 'C':
                        respObj.addContext(s);
                        break;
                    case 'R':
                        respObj.addResp(s);
                        break;
                    case '%':
                        ListOfRespObj.add(respObj);
                        respObj.clear();
                        break;
                    case '#':
                        if(ListOfRespObj.isEmpty()){
                            ListOfRespObj.add(respObj);
                        }
                        for(String sn:keyList){
    ...
    public static String get_input(String nn) throws Exception {
        System.out.print(">");
...
//            String[] contextList = KnowledgeBase[i][2];
...
//                    }
    ...
    static boolean wrong_context(String[] contextList) {
        boolean bWrongContext = true;
        if (contextList.length == 0) {
            bWrongContext = false;
        } else {
            String sContextTemp = sPrevResponse;
            cleanString(sContextTemp);
            for (int i = 0; i < contextList.length; i++) {
                if (contextList[i].equals(sContextTemp)) {
                    sPrevContext = sContext;
                    sContext = sContextTemp;
                    bWrongContext = false;
                    break;
                }
            }
...
//            System.out.println(sResponse);
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            while (!quit()) {
//                            get_input();
                System.out.println(get_input("i want ice-cream"));
...
class CResponse{
    private List<String> ListOfContext=new ArrayList<String>();
    private List<String> ListOfResp=new ArrayList<String>();

    public void addContext(String s)
    {
        ListOfContext.add(s);
    }
    public void addResp(String s)
    {
        ListOfResp.add(s);
    }
