...
// All Record
    ...
    public String editData() {
        myList = (List<UserBean>) ActionContext.getContext().getSession().get("list");
        if (oper.equalsIgnoreCase("add")) {
            for (UserBean u : myList) {
                if (u.getUname().equalsIgnoreCase(uname)) {
                    errmsg = "errorhere : Username already exists";
                    return ERROR;
                }
            }
            ...
            myList.add(temp);
//            gridModel=myList;
            ActionContext.getContext().getSession().put("list", myList);
        }
        return SUCCESS;
    }
